...
def visualize_data(x_axis, data, title, x_label, y_label, legend_labels):
    for i, data_column in enumerate(data):
        plt.plot(x_axis, data_column, label=legend_labels[i])
    plt.title(title)
    plt.xlabel(x_label)
    plt.ylabel(y_label)
...
data = pd.read_html(insolvenz_url)[0][:-1]
date_column = data[data.columns[0]] + " " + data[data.columns[1]]
date_column = date_column.apply(
    lambda x: convert_month_year_date_to_datetime_object(x)
).tolist()
...
legend_labels = ["Insolvencies", "Companies", "Consumer"]

visualize_data(
    date_column,
    data=data,
    title="Insolvenzen Deutschland",
    x_label="Months",
    y_label="Insolvencies (total)",
