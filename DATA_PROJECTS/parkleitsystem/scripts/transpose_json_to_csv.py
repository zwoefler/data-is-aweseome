...
def transpose_json_to_csv(json_path):
    ...
    with open(csv_file, mode='w', newline='') as file:
        ...
        for entry in data["occupation_data"]:
            ...
            writer.writerow([hhmm_time, entry["free_spaces"], entry["occupied_spaces"], entry["max_spaces"]])
    
    print(f'Data has been written to {csv_file}')

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Transpose JSON to CSV.')
    parser.add_argument('json_path', type=str, help='Path to the JSON file to be transposed.')
